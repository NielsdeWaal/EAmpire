...
class Enemy {
  private:
    ...
    */
    void corner_check(sf::Vector2f &boundarieA, sf::Vector2f &boundarieB,
    ...
    // The start boundary that have to be located: default first tile(most left
    // at top)
  ...
  public:
    ...
    * @brief Constructor
    *
    * If this constructor is called, it will make a circle shaped enemy with
    * custom characteristics.
    * That are determines by the parameters. It will also automatic set the
    * radius, color and position.
    *
    * @param[in]	color	The default color of the enemy.
    * @param[in]	damage	The damage the enemy will do to the players
    * lives.
    ...
    *
    * @param[in]	damage_tower	The damage of the tower who reduced the lives
    * of the enemy
    */
    ...
    *
    * The direction to move your enemy into is simply the difference between the
    * location and the enemy position.
    * However, you want the enemies to move with constant speed, so we normalize
    * the result.
    * This will give you the direction as a vector of length 1. multiply this
    * direction by the speed constant of that enemy's type.
    * The result is a vector with it's length depending on the speed factor
    * instead of the distance to the player.
    *
    * @param[in]	size_grid	The size of the grid that is used when position
    * of enemy is outside the grid
    */
    ...
    *
    * @param[in]	rhs	The vector2i what have to be
    * transformed
    */
    ...
    * When a tile matches the boundaryA and the next tile is not the end of the
    * path.
    *
    ...
    * If the next tile is not the last tile. BoundaryB becomes the next location
    * in the path.
    *
    * If the position of the enemy is within the boundaries. the enemy moves in
    * the direction of boundaryB.
    *
    * otherwise boundaryA becomes boundaryB. If the next tile is not the last
    * tile.
    * BoundaryB becomes the next location in the path. and then the enemy moves
    * in the direction of boundaryB and executes a corner check
    *
    * untrue, true will be returned
    *
    * @param[in]	path	The path for looking at the next pathtile
    * @return		bool	When the end destination is reached, true will be 
    * returned.
    */
    ...
    *
    * @param[out]	window	The screen on which you have to
    * draw
    */
