...
int main(void) {
    ...
    Button exit_button(exit,
                       sf::Vector2f{(float((window.getSize().x / 2) + 212)),
                                    (float((window.getSize().y / 2) + 100)) +
                                        window.getSize().y * 0.15f},
                       sf::Vector2f{70, 50}, window);
    Button basic_tower(
        tower1,
        sf::Vector2f{(float((window.getSize().x / 2) + 325)),
                     (float((window.getSize().y - window.getSize().y) + 77))},
        sf::Vector2f{130, 50}, window);
    Button sell_button(
        sell,
        sf::Vector2f{(float((window.getSize().x / 2) + 325)),
                     (float((window.getSize().y - window.getSize().y) + 137))},
        sf::Vector2f{130, 50}, window);
    ...
    action actions[] = {
        ...
        action(sf::Mouse::Left, [&state, &window, &exit_button, &basic_tower,
                                 &sell_button, &grid] {
